...
import { sendPpidResponse } from "@/lib/actions/layanan.actions";
...
interface PpidResponseModalProps {
  requestId: string;
  email: string | null;
}

export function PpidResponseModal({ requestId, email }: PpidResponseModalProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [state, action] = useActionState(sendPpidResponse, { success: false, error: "" });
  ...
  useEffect(() => {
    if (state.success) {
      ...
      if (state.warning) {
        ...
        showToast("Tanggapan Berhasil", "Dokumen PPID/Tanggapan berhasil dikirim via email.", "success");
      }
  ...
  if (!email) {
    return (
      <button disabled className="flex items-center gap-2 text-xs font-semibold text-gray-400 cursor-not-allowed mt-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6 dark:bg-navy-dark dark:border dark:border-white/10 text-left">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-navy dark:text-white">Balas Permohonan PPID</h3>
            ...
            <form action={action} className="space-y-4">
              ...
              <div>
                <label className="mb-1 block text-sm font-semibold text-navy dark:text-white">Isi Balasan / Lampiran Teks <span className="text-rose-500">*</span></label>
                <textarea name="response_message" rows={6} required placeholder="Berikan penjelasan atau tautan dokumen yang diminta..." className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm outline-none focus:border-primary dark:bg-navy dark:border-white/10 dark:text-white" />
