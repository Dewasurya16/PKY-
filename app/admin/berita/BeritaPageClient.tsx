...
import { createNews, updateNews, deleteNews } from "@/lib/actions/news.actions";
import type { News, ActionState } from "@/lib/types/database";
...
function NewsForm({ editItem, onClose, onSuccess }: NewsFormProps) {
  const isEditing = Boolean(editItem);

  const action = isEditing ? updateNews : createNews;
    ...
    <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-white/10 dark:bg-navy">
      ...
      {state.success && (
        <div className="mb-4 rounded-xl bg-emerald-50 p-3 text-sm font-medium text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">
          Berita berhasil {isEditing ? "diperbarui" : "ditambahkan"}!
        </div>
      )}
      ...
      <form action={formAction} className="space-y-5">
        ...
        <FormField
          ...
          hint="Ditampilkan sebagai preview di halaman daftar berita"
        >
          <textarea
            id="excerpt"
            name="excerpt"
        ...
        <FormField
          ...
          hint="Pisahkan paragraf dengan baris kosong (Enter 2x)"
        >
          <textarea
            id="content"
            name="content"
        ...
        <FormField label="Gambar Thumbnail" name="image">
          <FileUpload
            name="image"
        ...
        >
          <input
            ...
            type="datetime-local"
            defaultValue={
              editItem?.published_at
                ? new Date(editItem.published_at).toISOString().slice(0, 16)
                : ""
            }
            className={INPUT_CLASS}
          />
        ...
        <FormField label="Status" name="is_published">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="hidden"
              name="is_published"
              value={editItem?.is_published ? "true" : "false"}
            />
            <input
              type="checkbox"
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 dark:border-white/10">
          ...
          <SubmitButton
            label={isEditing ? "Perbarui Berita" : "Simpan Berita"}
...
function DeleteButton({ item }: { item: News }) {
  const [state, formAction, isPending] = useActionState<ActionState, FormData>(
    deleteNews,
    { success: false },
  );
...
// ──────────────────────────── Main Page (Client Wrapper) ────────────────────────────

type BeritaPageClientProps = {
  initialData: News[];
};
...
export function BeritaPageClient({ initialData }: BeritaPageClientProps) {
  ...
  // Matikan loading spinner jika data baru dari server sudah masuk
  useEffect(() => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary dark:text-white/60">
            Kelola semua berita dan informasi publik
          </p>
        </div>
        {!isFormOpen && (
          <button
      ...
      {/* Form */}
      {isFormOpen && (
        <NewsForm editItem={editItem} onClose={handleClose} onSuccess={handleSuccess} />
      )}

      {/* Table */}
      <DataTable<News & Record<string, unknown>>
        isLoading={isRefetching}
        data={initialData as (News & Record<string, unknown>)[]}
        searchKey="title"
        ...
        columns={[
          ...
          },
          {
            ...
            label: "Kategori",
            render: (item) => (
              <span className="rounded-lg bg-primary/10 px-2.5 py-1 text-xs font-semibold text-primary dark:bg-primary/20 dark:text-primary-light">
          ...
          },
          {
            ...
            render: (item) => (
              <span className="text-sm text-text-secondary dark:text-white/60">
                {new Date(item.published_at as string).toLocaleDateString("id-ID", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                })}
        ...
        onRowAction={(item) => (
          <div className="flex items-center gap-1">
            <button
              onClick={() => handleEdit(item as unknown as News)}
            ...
            <DeleteButton item={item as unknown as News} />
          </div>
        )}
      />
    </div>
